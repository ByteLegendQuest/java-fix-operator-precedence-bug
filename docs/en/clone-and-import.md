...
`git clone https://github.com/ByteLegendQuest/java-fix-operator-precedence-bug`

A project directory named `java-fix-operator-precedence-bug` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-operator-precedence-bug` directory which is created just now.
